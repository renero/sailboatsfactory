...
class CPT:
    ...
    root = None
    # Inverted Index dictionary, where key : unique item, value : set of
    # sequences containing this item
    II = None
    # A Lookup table dictionary, where key : id of a sequence(row),
    # value: leaf node of a Prediction Tree
    LT = None
    ...
    def load_files(self, train_file, test_file, merge=False):
        """
        This function reads in the wide csv file of sequences separated by
        commas and returns a list of list of those
        ...
        """
        # List of list containing the entire sequence data using which the
        # model will be trained.
        train = []
        # List of list containing the test sequences whose
        # next n items are to be predicted
        test = []
    ...
    def train(self, train):
        """
        This functions populates the Prediction Tree, Inverted Index and
        LookUp Table for the algorithm.
        ...
        for seqid, row in enumerate(train):
            for element in row:
                ...
                if cursornode.hasChild(element) is False:
    ...
    def score(self, counttable, key, length, target_size,
              number_of_similar_sequences, number_items_counttable):
        """
        This function is the main workhorse and calculates the score to be
        populated against an item. Items are predicted
        using this score.

        Output: Returns a counttable dictionary which stores the score against
        items. This counttable is specific for a
        particular row or a sequence and therefore re-calculated at each
        prediction.
    ...
    def predict(self, train, test, k, n=1):
        ...
        k is the number of last elements that will be used to find similar
        sequences and,
        ...
        for each_target in tqdm(test, ascii=True):
            ...
            for sequence in similar_sequences:
                try:
                    index = next(i for i, v in zip(
                        range(len(sequence) - 1, 0, -1), reversed(sequence))
                        if v == each_target[-1])
                except Exception:
                    index = None
                if index is not None:
                    ...
                    for element in sequence[index + 1:]:
                        ...
                        counttable = self.score(counttable, element, len(
                            each_target), len(each_target),
    ...
    def get_n_largest(self, dictionary, n):
        """
        A small utility to obtain top n keys of a Dictionary based on
        their values.
        """
        largest = sorted(dictionary.items(),
